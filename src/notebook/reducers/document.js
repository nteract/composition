...
import { emptyCodeCell, emptyMarkdownCell } from "../../../packages/commutable";
...
import { createImmutableOutput } from "../../../packages/commutable/v4";

import type { Output, StreamOutput } from "../../../packages/commutable/v4";

type Pager = { source: "page", data: MimeBundle, start: number };
...
export function cleanCellTransient(state: DocumentState, id: string) {
  ...
  return state
    .updateIn(["transient", "keyPathsForDisplays"], (
...
function appendOutput(state: DocumentState, action: AppendOutputAction) {
  ...
  const keyPaths: KeyPaths = state // Extract the current list of keypaths for this displayID
    .getIn(

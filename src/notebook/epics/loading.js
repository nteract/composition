...
/**
  ...
  */
export const loadEpic = (actions: ActionsObservable) => actions
  .ofType(LOAD)
  .do(action => {
  ...
  .switchMap(action => readFileObservable(action.filename)
    ...
    .catch(err => Observable.of({ type: "ERROR", payload: err, error: true })));
...
export const newNotebookEpic = (action$: ActionsObservable) =>
  action$.ofType(NEW_NOTEBOOK).switchMap(action => Observable.of(
    {
      type: "SET_NOTEBOOK",

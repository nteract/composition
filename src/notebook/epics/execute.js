...
export const createErrorActionObservable = type =>
  error => Rx.Observable.of({ type, payload: error, error: true });
...
export function msgSpecToNotebookFormat(msg) {
  return Object.assign({}, msg.content, { output_type: msg.header.msg_type });
}
...
export function executeCellEpic(action$, store) {
  return action$
    ...
    .map(cellActionStream =>
      cellActionStream.switchMap(({ source, id }) =>

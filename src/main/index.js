...
const prepJupyterObservable = prepareEnv
  .mergeMap(() => // Create all the directories we need in parallel
  ...
  .mergeMap(() => readFileObservable(nteractConfigFilename).catch(err => {
    if (err.code === "ENOENT") {
...
const openFile$ = Rx.Observable.fromEvent(app, "open-file", (
  event,
  filename
) => ({ event, filename }));
